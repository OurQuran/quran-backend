...
class BaseWordArtifactsGenerator
{
    public function generate(Command $command, array $options = []): int
    {
        ...
        $q = DB::table('words as w')
            ->select('w.id', 'w.word', 'w.pure_word', 'w.word_template')
            ->orderBy('w.id');

        $command->info(
            "Generating base word artifacts... mode={$mode} (chunk={$chunk}, batch={$batch}, " .
            ($dry ? 'DRY-RUN' : 'WRITE') . ")"
        );
        ...
        $q->chunkById($chunk, function ($words) use (
            ...
            foreach ($words as $word) {
                ...
                $wordText = trim((string) ($word->word ?? ''));
                if ($wordText === '') {
                    continue;
                }
                ...
                if ($generateHtml) {
                    ...
                    if ((string) ($word->word_template ?? '') !== $tpl) {
                        $pendingHtml[$id] = $tpl;
                        $htmlUpdated++;
                    }
                }
