...
class ImportQiraatMushafAyahsFromExcel extends Command
{
    ...
    protected $signature = 'qiraat:import-mushaf-ayahs-excel
        {qiraat_reading : qiraat_readings.id, stable code, OR "auto" to import all mappings}
        {path? : Excel file (.xlsx), multiple files separated by comma, or a directory. If omitted or "auto", uses mapping for that reading code}
    ...
    {
        $qiraatArg = $this->argument('qiraat_reading');
        ...
        if (strtolower($qiraatArg) === 'auto') {
            ...
            $resolved = QiraatImportMaps::resolveReadingIdsForCodes(array_keys(QiraatImportMaps::mushafExcelByReadingCode()));
            $this->line("Mapped qiraat ids: " . implode(', ', array_values($resolved)));
            $this->line("Sheet: {$sheetIndex} | Chunk: {$chunk} | Mode: " . ($dryRun ? 'DRY-RUN' : 'INSERT'));

            $ok = 0;
            $fail = 0;

            foreach ($resolved as $code => $qiraatId) {
                $this->newLine();
                $this->info("=== Import qiraat_reading={$code} (id={$qiraatId}) ===");
        ...
        // Single mode
        $qiraatId = QiraatImportMaps::resolveReadingId($qiraatArg);
        if (!$qiraatId) {
            $this->error("qiraat_readings not found for reading={$qiraatArg}");
            return self::FAILURE;
        }
            ...
            ->chunk(2000, function ($ayahs) use (
                ...
                foreach ($ayahs as $ayah) {
                    ...
                    if (count($buffer) >= $chunk) {
                        $this->upsertMushafAyahRows($buffer, $dryRun);
                        $buffer = [];
    ...
    private function resolveExcelPathFromMap(int $qiraatId): string
    {
        return $this->resolveStorageMappedPath(
            $this->argument('path'),
            array_reduce(
                array_keys(QiraatImportMaps::mushafExcelByReadingCode()),
                function (array $carry, string $code): array {
                    $id = QiraatImportMaps::resolveReadingIdByCode($code);
                    if ($id !== null) {
                        $carry[$id] = QiraatImportMaps::mushafExcelByReadingCode()[$code];
                    }
                    return $carry;
                },
                []
    ...
    {
        $this->openCsvReport($path, 'qiraat_excel_' . $qiraatId . '_' . now()->format('Y-m-d_His') . '_report.csv', [
            'row_index',
            'surah_no',
            'aya_no',
            'reason',
