...
class AutoMapMushafWords extends Command
{
    ...
    private function runSingle(int $qiraatId, bool $onlyUnmapped, bool $dryRun, int $maxCombined, int $maxSplit): int
    {
        ...
        foreach ($mushafAyahs as $ma) {
            ...
            $ayMap = $this->loadAyahMappingGroup($mushafAyahId);
            if ($ayMap === null) {
            ...
            if (!$dryRun && !empty($rowsToUpsert)) {
                $this->saveWordMappingsUpsert($rowsToUpsert, $onlyUnmapped, $forceRemapMushafWordIds);
            }
        }
     ...
     */
    private function loadAyahMappingGroup(int $mushafAyahId): ?array
    {
        ...
        if ($type === 'split') {
            ...
            $group = DB::table('mushaf_ayah_to_ayah_map')
                ->where('ayah_id', $baseAyahId)
     ...
     * by (surah_id, number_in_surah) -> ayahs.id (MIN(id) when base has duplicates).
     * So you can run this command after importing from Excel without running map-mushaf-ayahs first.
     */
    private function ensureAyahMappingsForQiraat(int $qiraatId, bool $dryRun): int
    {
        if ($dryRun) {
            $count = DB::table('mushaf_ayahs as m')
        ...
        $basePick = DB::table('ayahs')
            ->selectRaw('surah_id, number_in_surah, MIN(id) as ayah_id')
            ->groupBy('surah_id', 'number_in_surah');

        $unmapped = DB::table('mushaf_ayahs as m')
            ->joinSub($basePick, 'bp', function ($j) {
                $j->on('bp.surah_id', '=', 'm.surah_id')->on('bp.number_in_surah', '=', 'm.number_in_surah');
            })
            ->where('m.qiraat_reading_id', $qiraatId)
            ->whereNotNull('m.surah_id')
            ->whereNotNull('m.number_in_surah')
            ...
            ->select('m.id as mushaf_ayah_id', 'bp.ayah_id')
            ->get();
        ...
        $now = now();
        $rows = $unmapped->map(function ($r) use ($now) {
            return [
     ...
     */
    private function saveWordMappingsUpsert(array $rowsToUpsert, bool $onlyUnmapped, array $forceRemapMushafWordIds): void
    {
        DB::transaction(function () use ($rowsToUpsert, $onlyUnmapped, $forceRemapMushafWordIds) {

            $forceIds = array_keys($forceRemapMushafWordIds);

            if (!$onlyUnmapped) {
                $allMushafIds = array_values(array_unique(array_map(fn($r) => (int)$r['mushaf_word_id'], $rowsToUpsert)));
                $deleteIds = array_values(array_unique(array_merge($allMushafIds, $forceIds)));
