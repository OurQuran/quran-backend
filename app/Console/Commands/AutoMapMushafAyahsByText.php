    ...
    private function runSingle(int $qiraatId, bool $onlyUnmapped, bool $dryRun, int $maxCombined, int $maxSplit): int
    {
        ...
        for ($surahId = 1; $surahId <= 114; $surahId++) {
            ...
            if (!$dryRun && !empty($toUpsert)) {
                $this->saveMappingsUpsert($remapMushafIds, $toUpsert, $onlyUnmapped, $forceRemapIds);
            }
        }
...
DO UPDATE SET
  ...
  updated_at  = EXCLUDED.updated_at
     ...
     * ----------------------------
     */
    private function saveMappingsUpsert(array $remapIds, array $toUpsert, bool $onlyUnmapped, array $forceRemapIds = []): void
    {
        DB::transaction(function () use ($remapIds, $toUpsert, $onlyUnmapped, $forceRemapIds) {
